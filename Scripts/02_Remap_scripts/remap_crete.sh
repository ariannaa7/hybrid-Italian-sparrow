...
#SBATCH -e ../Job_logs/02_Remap_logs/remapping_crete.err

#SBATCH -J remap_existingBAM_crete
...
mkdir /proj/sparrowhybridization/Pitaliae/working/Arianna/02_Remap/crete_remap
...
for bam in ../Data/Bamfiles/crete/*.bam ; do # for each file in the directory that ends in .bam

    prefix=$(basename "$bam" .bam | cut -d "_" -f 1) # Pull the basename then prefix, e.g. "C060_resorted_nodup_realigned" then just "C060"
    ...
    samtools collate -Oun128 "${bam}" -@ 20 | samtools fastq -OT RG,BC -@ 20 - \
        ...
        | samtools sort -@ 20 -m4g -o "crete_remap/${prefix}_remapped.bam" -
