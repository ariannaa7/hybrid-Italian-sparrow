...
#SBATCH -e ../Job_logs/02_Remap_logs/remapping_p_montanus.err

#SBATCH -J remap_existingBAM_p_montanus
...
mkdir /proj/sparrowhybridization/Pitaliae/working/Arianna/02_Remap/p_montanus_remap
...
for bam in ../Data/Bamfiles/p_montanus/*.bam ; do # for each file in the directory that ends in .bam

    prefix=$(basename "$bam" .bam | cut -d "_" -f 1) # Pull the basename then prefix, e.g. "PM007_merge_sort_realigned" then just "PM007"
    ...
    samtools collate -Oun128 "${bam}" -@ 20 | samtools fastq -OT RG,BC -@ 20 - \
        ...
        | samtools sort -@ 20 -m4g -o "p_montanus_remap/${prefix}_remapped.bam" -
