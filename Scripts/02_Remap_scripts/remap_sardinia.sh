...
mkdir /proj/sparrowhybridization/Pitaliae/working/Arianna/02_Remap/sardinia_remap
...
for bam in ../Data/Bamfiles/sardinia/*.bam ; do # for each file in the directory that ends in .bam

    prefix=$(basename "$bam" .bam | cut -d "_" -f 1) # Pull the basename then prefix, e.g. "SA001_merge_sort_realigned" then just "SA001"
    ...
    samtools collate -Oun128 "${bam}" -@ 20 | samtools fastq -OT RG,BC -@ 20 - \
        ...
        | samtools sort -@ 20 -m4g -o "sardinia_remap/${prefix}_remapped.bam" -
