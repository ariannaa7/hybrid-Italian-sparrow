...
#SBATCH -e ../Job_logs/02_Remap_logs/remapping_miscellaneous.err

#SBATCH -J remap_existingBAM_miscellaneous
...
mkdir /proj/sparrowhybridization/Pitaliae/working/Arianna/02_Remap/miscellaneous_remap
...
for bam in ../Data/Bamfiles/miscellaneous/*.bam ; do # for each file in the directory that ends in .bam

    prefix=$(basename "$bam" .bam | cut -d "_" -f 1) # Pull the basename then prefix, e.g. "8934547_merge_sort_realigned" then just "8934547"
    ...
    samtools collate -Oun128 "${bam}" -@ 20 | samtools fastq -OT RG,BC -@ 20 - \
        ...
        | samtools sort -@ 20 -m4g -o "miscellaneous_remap/${prefix}_remapped.bam" -
