...
#SBATCH -e ../Job_logs/02_Remap_logs/remapping_rimini.err

#SBATCH -J remap_existingBAM_rimini
...
mkdir /proj/sparrowhybridization/Pitaliae/working/Arianna/02_Remap/rimini_remap
...
for bam in ../Data/Bamfiles/rimini/*.bam ; do # for each file in the directory that ends in .bam

    prefix=$(basename "$bam" .bam) # Pull the basename, e.g. "Rimini_110"
    ...
    samtools collate -Oun128 "${bam}" -@ 20 | samtools fastq -OT RG,BC -@ 20 - \
        ...
        | samtools sort -@ 20 -m4g -o "rimini_remap/${prefix}_remapped.bam" -
