...
#SBATCH -e ../Job_logs/02_Remap_logs/remapping_guglionesi.err

#SBATCH -J remap_existingBAM_guglionesi
...
mkdir /proj/sparrowhybridization/Pitaliae/working/Arianna/02_Remap/guglionesi_remap
...
for bam in ../Data/Bamfiles/guglionesi/*.bam ; do # for each file in the directory that ends in .bam

    prefix=$(basename "$bam" .bam) # Pull the basename, e.g. "Guglionesi429"
    ...
    samtools collate -Oun128 "${bam}" -@ 20 | samtools fastq -OT RG,BC -@ 20 - \
        ...
        | samtools sort -@ 20 -m4g -o "guglionesi_remap/${prefix}_remapped.bam" -
