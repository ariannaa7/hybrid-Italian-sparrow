...
#SBATCH -e ../Job_logs/02_Remap_logs/remapping_spanish.err

#SBATCH -J remap_existingBAM_spanish
...
mkdir /proj/sparrowhybridization/Pitaliae/working/Arianna/02_Remap/spanish_remap
...
for bam in ../Data/Bamfiles/spanish/*.bam ; do # for each file in the directory that ends in .bam

    prefix=$(basename "$bam" .bam) # Pull the basename e.g. "Lesina_286"
    ...
    samtools collate -Oun128 "${bam}" -@ 20 | samtools fastq -OT RG,BC -@ 20 - \
        ...
        | samtools sort -@ 20 -m4g -o "spanish_remap/${prefix}_remapped.bam" -
