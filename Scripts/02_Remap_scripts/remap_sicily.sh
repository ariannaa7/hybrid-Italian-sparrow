...
#SBATCH -e ../Job_logs/02_Remap_logs/remapping_sicily.err

#SBATCH -J remap_existingBAM_sicily
...
mkdir /proj/sparrowhybridization/Pitaliae/working/Arianna/02_Remap/sicily_remap
...
for bam in ../Data/Bamfiles/sicily/*.bam ; do # for each file in the directory that ends in .bam

    prefix=$(basename "$bam" .bam | cut -d "_" -f 1) # Pull the basename then prefix, e.g. "S029_resorted_nodup_realigned" then just "S029"
    ...
    samtools collate -Oun128 "${bam}" -@ 20 | samtools fastq -OT RG,BC -@ 20 - \
        ...
        | samtools sort -@ 20 -m4g -o "sicily_remap/${prefix}_remapped.bam" -
